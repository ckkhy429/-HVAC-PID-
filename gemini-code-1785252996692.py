...
st.caption("승객 체감 피크백(더워요/추워요) 연동 실시간 PID 냉방 제어 시뮬레이터")
...
if total_votes > 0:
    ...
    # '더워요'가 많으면 목표 온도를 최대 -0.5°C 낮춤, '추워요'가 많으면 +0.5°C 올림
...
m3.metric("승객 피크백 보정 온도", f"{T_TARGET:.2f}°C", delta=f"{target_offset:.2f}°C")
m4.metric("총 참여 승객 수", f"{total_votes} 명")

# 4. PID 제어 시뮬레이션 연산
hours = np.arange(5, 25)
# 2호선 기준 혼잡도 곡선 (노선별 가중치 적용)
...
time_min = np.arange(0, 19 * 60, 1.0) # 1분 간격
hours_cont = 5.0 + time_min / 60.0
passengers = np.interp(hours_cont, hours, [c * 1.6 for c in crowding_pct]) # 정원 160명 기준
...
# 1단: 혼잡도
ax[0].plot(hours_cont, crowding_pct, color="orange", label="Crowding Rate (%)")
...
ax[1].grid(alpha=0.3)import streamlit as st
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
# 4. PID 제어 시뮬레이션 연산
hours = np.arange(5, 25) # 05시 ~ 24시 (20개 포인트)
line_weight = {"1호선": 0.8, "2호선": 1.0, "3호선": 0.85, "4호선": 0.95, "5호선": 0.8, "6호선": 0.65, "7호선": 0.9, "8호선": 0.8, "9호선": 1.1}
base_crowding = [12, 35, 88, 150.4, 95, 55, 48, 52, 50, 53, 58, 65, 92, 145.2, 110, 70, 65, 55, 35, 15]
crowding_pct = [c * line_weight[selected_line] for c in base_crowding]

time_min = np.arange(0, 19 * 60, 1.0) # 1분 간격 (1,140개 데이터)
hours_cont = 5.0 + time_min / 60.0

# [수정 포인트] 혼잡도와 승객 수를 연속 데이터(1,140개)로 보간하여 크기 맞춤
crowding_pct_cont = np.interp(hours_cont, hours, crowding_pct)
passengers = crowding_pct_cont * 1.6 # 정원 160명 기준
